  ...
  grunt.registerTask('exportDigikam', 'export the digikam photo data', function(dbFile){
    if (arguments.length === 0) {
      ...
      db.serialize(function() {
        ...
        db.each("SELECT i.id, a.relativePath||'/'||i.name AS src , GROUP_CONCAT(it.tagid) AS tagIds FROM Images i,Albums a, ImageTags it WHERE a.id = i.album AND it.imageid = i.id GROUP BY i.id", function(err, row) {
            listImg.push('  "'+row.id + '": { "src": "'+row.src+'", "tagIds": ['+row.tagIds+']}');
        }, function(){
        ...
        'SELECT * FROM ImageTagProperties', function(err, row) {
            ...
            listImgTags.push('  "'+row.imageid+","+row.tagid + '": { "rect": { "x": '+x+', "y": '+y+', "width": '+width+', "height": '+height+'}}');
        }, function(){
